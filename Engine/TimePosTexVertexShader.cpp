...
PLUGIN_API
TimePosTexVertexShader::TimePosTexVertexShader(const std::wstring& compiledShaderPath)
...
void TimePosTexVertexShader::Bind() const
{
...
PLUGIN_API
void TimePosTexVertexShader::SetWorldMatrix(const XMFLOAT4X4& worldMatrix) // row major
{
...
PLUGIN_API
void TimePosTexVertexShader::SetViewProjectionMatrix(const XMFLOAT4X4& viewProjectionMatrix) // row major
{
...
PLUGIN_API
void TimePosTexVertexShader::SetTime(float time)
{
	if (m_CameraConstantBuffer.data.time == time)
		return;

	m_CameraConstantBuffer.data.time = time;
	m_CameraConstantBuffer.ApplyChanges();
}